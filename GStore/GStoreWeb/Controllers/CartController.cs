...
{
	public class CartController : AreaBaseController.RootAreaBaseController
	{
		...
		public ActionResult UpdateQty(string id, int? quantity, string type)
		{
			...
			if (product == null)
			{
				AddUserMessage("Update Quantity Error", "Item '" + id.ToHtml() + "' cannot be added to your cart. Please try again.", UserMessageType.Danger);
				return RedirectToAction("Index");
			}
		...
		[HttpGet]
		public ActionResult UpdateBundleQty()
		{
			//stub for URL hacks
			return RedirectToAction("Index");
		}
