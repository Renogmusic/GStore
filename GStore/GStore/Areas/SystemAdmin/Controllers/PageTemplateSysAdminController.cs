...
namespace GStore.Areas.SystemAdmin.Controllers
{
	public class PageTemplateSysAdminController : BaseClasses.SystemAdminBaseController 
    {
        ...
        {
			IQueryable<PageTemplate> query = GStoreDb.PageTemplates.All();
        ...
        public ActionResult Create(PageTemplate pageTemplate)
        {
            ...
            if (ModelState.IsValid)
            {
				...
				AddUserMessage("Page Template Added", "Page Template '" + pageTemplate.Name + "' [" + pageTemplate.PageTemplateId + "] created successfully!", AppHtmlHelpers.UserMessageType.Success);
		...
		public ActionResult Edit(PageTemplate pageTemplate)
		{
            ...
            if (ModelState.IsValid)
			{
				...
				AddUserMessage("Page Template changes saved", "Changes to Page Template '" + pageTemplate.Name + "' [" + pageTemplate.PageTemplateId + "] saved successfully.", AppHtmlHelpers.UserMessageType.Success);
				GStoreDb.SaveChanges();
        ...
        public ActionResult DeleteConfirmed(int id)
        {
			try
			{
				...
				if (deleted)
				{
					AddUserMessage("Page Template Deleted", "Page Template '" + pageTemplateName + "' [" + id + "] was deleted successfully. Sections deleted: " + sectionsToDelete.Count + ".", AppHtmlHelpers.UserMessageType.Success);
				}
			}
			catch (Exception ex)
			{
		...
		public ActionResult SectionEdit(PageTemplateSection section)
		{
			...
			if (ModelState.IsValid)
			{
				...
				GStoreDb.SaveChanges();
				AddUserMessage("Page Template Section Updated", "Changes saved successfully to Page Template Section '" + Server.HtmlEncode(section.Name) + "' [" + section.PageTemplateSectionId + "]", AppHtmlHelpers.UserMessageType.Success);
				return RedirectToAction("SectionIndex", new { id = section.PageTemplateId });
			}
